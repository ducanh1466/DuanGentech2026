<div class="admin-table-card">
        ...
        <div class="d-flex gap-2 align-items-center flex-wrap">
            <form method="GET" class="d-flex gap-3 m-0 p-2 align-items-center flex-grow-1" style="background: #f8fafc; border-radius: 12px; border: 1px solid #e2e8f0;">
                ...
                <div class="input-group input-group-sm" style="flex: 1; min-width: 160px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); border-radius: 8px; overflow: hidden;">
                    ...
                    <select name="stock_filter" class="form-select border-start-0 border-light shadow-none fw-medium" onchange="this.form.submit()" style="color: #475569; cursor: pointer;">
                        <option value="">Tất cả trạng thái</option>
                        <option value="in" <?= (isset($_GET['stock_filter']) && $_GET['stock_filter'] == 'in') ? 'selected' : '' ?>>Còn hàng (> 10)</option>
                        <option value="low" <?= (isset($_GET['stock_filter']) && $_GET['stock_filter'] == 'low') ? 'selected' : '' ?>>Sắp hết (<= 10)</option>
                        <option value="out" <?= (isset($_GET['stock_filter']) && $_GET['stock_filter'] == 'out') ? 'selected' : '' ?>>Hết hàng (0)</option>
                    </select>
                </div>
                ...
                <div class="input-group input-group-sm" style="flex: 2; min-width: 200px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); border-radius: 8px; overflow: hidden;">
                    ...
                    <input type="text" name="keyword" class="form-control border-start-0 border-light shadow-none fw-medium" value="<?= htmlspecialchars($_GET['keyword'] ?? '') ?>" placeholder="Tìm tên SP, SKU..." style="color: #475569;">
                </div>
                ...
                <a href="<?= BASE_URL ?>?action=admin-inventory" class="btn btn-white btn-sm border border-light text-muted shadow-sm" style="border-radius: 8px; height: 31px; display: flex; align-items: center; justify-content: center;" title="Làm mới">
                        ...
                        <tr>
                            ...
                            <td>
                                <img src="<?= $item['image'] ? BASE_ASSETS_UPLOADS . 'products/' . $item['image'] : BASE_IMAGES . 'no-image.png' ?>" 
                                     class="img-thumbnail border-0" style="width: 45px; height: 45px; object-fit: cover; border-radius: 6px;" alt="">
                            </td>
                            <td>
                                <div class="fw-semibold text-dark" style="font-size: 0.95rem;"><?= htmlspecialchars($item['product_name']) ?></div>
                            ...
                            <td>
                                <span class="badge bg-light text-dark border"><?= htmlspecialchars($item['sku'] ?: 'N/A') ?></span>
                            </td>
                            <td class="text-center">
                                <?php if ($item['stock'] == 0): ?>
                                    <span class="badge bg-danger stock-display" data-variant-id="<?= $item['variant_id'] ?>">Hết hàng (0)</span>
                                <?php elseif ($item['stock'] <= 10): ?>
                                    <span class="badge bg-warning text-dark stock-display" data-variant-id="<?= $item['variant_id'] ?>">Sắp hết (<?= $item['stock'] ?>)</span>
                                <?php else: ?>
                                    <span class="badge bg-success stock-display" data-variant-id="<?= $item['variant_id'] ?>">Còn hàng (<?= $item['stock'] ?>)</span>
                                <?php endif; ?>
                            </td>
                            <td>
    ...
    <div class="d-flex justify-content-between align-items-center p-3 border-top" style="border-color:var(--border-light)!important">
        <div class="d-flex align-items-center gap-2">
            <select class="form-select form-select-sm" style="width: auto; border-radius: 4px;" onchange="window.location.href='?action=admin-inventory&keyword=<?= urlencode($keyword ?? '') ?>&stock_filter=<?= urlencode($_GET['stock_filter'] ?? '') ?>&category_id=<?= urlencode($category_id ?? '') ?>&limit='+this.value">
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination pagination-sm mb-0">
                        <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                            <a class="page-link" href="?action=admin-inventory&keyword=<?= urlencode($keyword ?? '') ?>&stock_filter=<?= urlencode($_GET['stock_filter'] ?? '') ?>&category_id=<?= urlencode($category_id ?? '') ?>&limit=<?= $limit ?? 10 ?>&page=<?= ($page ?? 1) - 1 ?>">
                        ...
                        for ($i = $start; $i <= $end; $i++): ?>
                            <li class="page-item <?= ($page == $i) ? 'active' : '' ?>">
                                <a class="page-link" href="?action=admin-inventory&keyword=<?= urlencode($keyword ?? '') ?>&stock_filter=<?= urlencode($_GET['stock_filter'] ?? '') ?>&category_id=<?= urlencode($category_id ?? '') ?>&limit=<?= $limit ?? 10 ?>&page=<?= $i ?>"><?= $i ?></a>
                            </li>
                        <?php endfor; 
      ...
      <div class="modal-body">
        ...
        <form id="stockForm">
            ...
            <div class="mb-3">
                ...
                <div class="form-text">Nhập số lượng bạn vừa nhập thêm vào kho. Tổng số lượng sẽ được cộng dồn.</div>
            </div>
        </form>
      </div>
