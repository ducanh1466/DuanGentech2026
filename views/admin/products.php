<div class="admin-table-card">
    ...
    <div class="d-flex justify-content-between align-items-center p-3 border-top" style="border-color:var(--border-light)!important">
        ...
        <div class="d-flex align-items-center gap-2">
            <select class="form-select form-select-sm" style="width: auto; border-radius: 4px;" onchange="window.location.href='?action=admin-products&keyword=<?= urlencode($keyword ?? '') ?>&limit='+this.value">
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination pagination-sm mb-0">
                        <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                            <a class="page-link" href="?action=admin-products&keyword=<?= urlencode($keyword ?? '') ?>&limit=<?= $limit ?? 10 ?>&page=<?= ($page ?? 1) - 1 ?>">
                        ...
                        if ($start > 1) {
                            echo '<li class="page-item"><a class="page-link" href="?action=admin-products&keyword='.urlencode($keyword ?? '').'&limit='.($limit ?? 10).'&page=1">1</a></li>';
                            if ($start > 2) {
                        ...
                        for ($i = $start; $i <= $end; $i++) {
                            ...
                            echo '<li class="page-item ' . $active . '"><a class="page-link" href="?action=admin-products&keyword='.urlencode($keyword ?? '').'&limit='.($limit ?? 10).'&page='.$i.'">' . $i . '</a></li>';
                        }
                        ...
                        if ($end < $totalPages) {
                            ...
                            echo '<li class="page-item"><a class="page-link" href="?action=admin-products&keyword='.urlencode($keyword ?? '').'&limit='.($limit ?? 10).'&page='.$totalPages.'">'.$totalPages.'</a></li>';
                        }
                        ?>
