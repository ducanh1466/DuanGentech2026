    ...
    <div class="hero-parallax-slider shadow-lg">
            ...
            <?php foreach ($heroBanners as $index => $banner): ?>
                <div class="hero-parallax-item <?= $index === 0 ? 'active' : '' ?>">
                    <div class="hero-parallax-bg"
                        style="background-image: url('<?= BASE_URL ?>assets/uploads/banner/<?= $banner['image_url'] ?>');">
                    </div>
                    ...
                    <div class="hero-parallax-content">
                        ...
                        <span
                            class="badge bg-primary rounded-pill px-3 py-2 mb-4 fs-6 shadow-sm border border-light border-opacity-25"
                            data-aos="fade-up" data-aos-delay="200"><?= htmlspecialchars($banner['title']) ?></span>
                        <!-- Nút bấm với link động -->
                        <div class="d-flex gap-3 mt-4">
                            <?php if (!empty($banner['link'])): ?>
                                <a href="<?= htmlspecialchars($banner['link']) ?>"
                                    class="btn btn-primary rounded-pill px-5 py-3 fw-bold fs-6 shadow-sm hover-elevate border-0"
                                    style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%);">Khám Phá Ngay</a>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
        ...
        <?php else: ?>
            ...
            <div class="hero-parallax-item active">
                <div class="hero-parallax-bg"
                    style="background-image: url('https://images.unsplash.com/photo-1611186871348-b1ce696e52c9?q=80&w=2070&auto=format&fit=crop');">
                </div>
                <div class="hero-parallax-overlay"></div>
                <div class="hero-parallax-content">
                    <span
                        class="badge bg-primary rounded-pill px-3 py-2 mb-4 fs-6 shadow-sm border border-light border-opacity-25"
                        data-aos="fade-up" data-aos-delay="200">GENTECH</span>
                    <h1 class="display-3 fw-bold mb-4 text-white" style="letter-spacing: -2px;">SẢN PHẨM <span
                            class="text-primary">CHÍNH HÃNG</span></h1>
                </div>
            </div>
        <?php endif; ?>
            ...
            <?php foreach ($latestProducts as $index => $product):
                ...
                <div class="col-sm-6 col-lg-3 mb-5" data-aos="fade-up" data-aos-delay="<?= ($index % 4 + 1) * 100 ?>">
                    <div class="product-card-premium h-100 d-flex flex-column">
                        <div class="card-img-wrap bg-light border-bottom"
                            style="border-color:#f1f5f9 !important; position: relative;">
                            <a href="<?= BASE_URL ?>?action=product-detail&id=<?= $product['product_id'] ?? 0 ?>"
                                class="d-block w-100 h-100">
                                <img src="<?= $imgUrl ?>" alt="<?= htmlspecialchars($product['product_name'] ?? 'Product') ?>"
                                    style="object-fit: cover; width: 100%; height: 100%;">
                            </a>
                            <div style="position: absolute; top: 10px; right: 10px; z-index: 999;">
                                <span class="badge bg-danger shadow-sm px-3 py-2 rounded-pill fw-bold"
                                    style="font-size:0.85rem;"><i class="bi bi-stars"></i> Mới</span>
                            </div>
                        ...
                        <div class="product-info p-4 flex-grow-1 d-flex flex-column">
                            ...
                            <a href="<?= BASE_URL ?>?action=product-detail&id=<?= $product['product_id'] ?? 0 ?>"
                                ...
                                <h5 class="fw-bold mb-1 text-truncate text-dark hover-primary" style="font-size:1.15rem;">
                                    <?= htmlspecialchars($product['product_name'] ?? 'Tên sản phẩm') ?></h5>
                            </a>
                            <p class="text-muted mb-3 text-truncate" style="font-size: 0.9rem;">
                                <?= htmlspecialchars($product['category_name'] ?? 'Danh mục') ?></p>
                            <div class="mt-auto d-flex justify-content-between align-items-center">
                                <span
                                    class="fw-bold fs-5 text-danger"><?= number_format($product['price'] ?? 0, 0, ',', '.') ?>đ</span>
                                <button class="btn btn-light rounded-circle icon-btn hover-elevate shadow-sm"
                                    style="width: 40px; height: 40px; display:flex; align-items:center; justify-content:center; border:1px solid #e2e8f0;">
        ...
        <?php if (!empty($promoBanners)): ?>
            <?php foreach ($promoBanners as $index => $banner): ?>
                <div class="col-md-6" data-aos="<?= $index % 2 == 0 ? 'fade-right' : 'fade-left' ?>">
                    <a href="<?= !empty($banner['link']) ? htmlspecialchars($banner['link']) : '#' ?>"
                        class="d-block rounded-4 overflow-hidden shadow-sm hover-zoom">
                        <img src="<?= BASE_URL ?>assets/uploads/banner/<?= $banner['image_url'] ?>"
                            alt="<?= htmlspecialchars($banner['title']) ?>" class="img-fluid w-100 rounded-4"
        ...
        <?php else: ?>
            ...
            <div class="col-md-6" data-aos="fade-right">
                <a href="#" class="d-block rounded-4 overflow-hidden shadow-sm hover-zoom">
                    <img src="<?= BASE_URL ?>assets/uploads/banner/bannersale1.jpg" alt="Banner Khuyến Mãi 1"
                        class="img-fluid w-100 rounded-4" style="height:250px; object-fit:cover;">
                </a>
            </div>
            <div class="col-md-6" data-aos="fade-left">
                <a href="#" class="d-block rounded-4 overflow-hidden shadow-sm hover-zoom">
                    <img src="<?= BASE_URL ?>assets/uploads/banner/bannersale2.jpg" alt="Banner Khuyến Mãi 2"
                        class="img-fluid w-100 rounded-4" style="height:250px; object-fit:cover;">
                </a>
            </div>
        <?php endif; ?>
                ...
                <div class="col-sm-6 col-lg-3 mb-5" data-aos="fade-up" data-aos-delay="<?= ($index % 4 + 1) * 100 ?>">
                    <div class="product-card-premium h-100 d-flex flex-column">
                        <div class="card-img-wrap bg-light border-bottom"
                            ...
                            <div style="position: absolute; top: 10px; right: 10px; z-index: 999;">
                                <span class="badge bg-dark shadow-sm px-3 py-2 rounded-pill fw-bold"
                                    style="font-size:0.85rem;"><i class="bi bi-fire text-warning"></i> Bán Chạy</span>
                            </div>
                            <button class="btn-quick-add hover-elevate">Thêm vào giỏ</button>
                        </div>
                        <div class="product-info p-4 flex-grow-1 d-flex flex-column">
